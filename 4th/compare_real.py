...
def main():
    ...
    # Initialize GFlowNet model (assuming ImprovedGFlowNet is imported from gflownet3.py)
    ...
    # Test 4: Correlation-Based Selection
    ...
    mses = [
        ensure_cpu(results['GFlowNet Average']['mse']),
        ensure_cpu(results['GFlowNet Best']['mse'])
    ] + [
        ensure_cpu(data['mse']) for m, data in results.items() 
        if m not in ['GFlowNet Average', 'GFlowNet Best']
    ]
